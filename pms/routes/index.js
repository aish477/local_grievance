  ...
  router.get('/departmentpage',checkloginuser,function(req,res,next){
   ...
   if(!req.session.user){
    ...
    var depart=departmentModule.findOne({email_id:loginUser});
     ...
     var complaint=compModule.find({$and: [{status:"forwarded"},
...
router.post('/forward/:id',function(req,res,next){
  ...
  var status=compModule.update({_id: id},{$set:
    {status:"forwarded"}
  });
...
[
  ...
  if (!errors.isEmpty()) {
      const user = matchedData(req);
      res.render('complaintform', { title: 'complaint registration form', msg: '', errors: errors.mapped(), user: user,ref:'' });
  } else {
